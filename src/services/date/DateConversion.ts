...
export default class DateConversion {
	static dateStringToUnixTime(value: string, dateFormat: DateFormat) {
	...
	static unixTimeToDateString = (unixTime: number, format: DateFormat) => {
	...
	static unixTimeToDateTimeString(unixTime: number, format: DateFormat) {
		const date = new Date(unixTime);
	...
	static isValidDateFormat(value: string, dateFormat: DateFormat) {
		switch (dateFormat) {
			case DateFormat.MM_DD_YYYY:
